...
void ActorMoveComponent::Update(float deltaTime)
{
	...
	if (! m_pModel->IsTileAccessible(glm::vec2{centerOfPlayer.x-tileSize, centerOfPlayer.y}))//left
	{
		newPos.x = std::max(newPos.x, static_cast<float> (static_cast<int>(centerOfPlayer.x / tileSize) * tileSize ));
	}


	if (!m_pModel->IsTileAccessible(glm::vec2{ centerOfPlayer.x + tileSize, centerOfPlayer.y }))//right
	{
		newPos.x = std::min(newPos.x, static_cast<float> (static_cast<int>(centerOfPlayer.x / tileSize) * tileSize));
	}

	if (!m_pModel->IsTileAccessible(glm::vec2{ centerOfPlayer.x , centerOfPlayer.y - tileSize }))//up
	{
		newPos.y = std::max(newPos.y, static_cast<float> (static_cast<int>(centerOfPlayer.y / tileSize) * tileSize));
	}

	if (!m_pModel->IsTileAccessible(glm::vec2{ centerOfPlayer.x , centerOfPlayer.y + tileSize }))//down
	{
