...
namespace dae
{
	...
	public:
		...
		static std::shared_ptr<PacmanComponent> Create(const std::shared_ptr<GameBoardModel>& pGameBoardModel)
		{
			auto ptr = std::shared_ptr<PacmanComponent>(new PacmanComponent(pGameBoardModel));
	...
	private:
		PacmanComponent(const std::shared_ptr<GameBoardModel>& pGameBoardModel);
